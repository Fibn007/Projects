...
template <class T> void SortedArray(T* data, int size);
template <class T> void ArrayOfZero(T* data, int size);
template <class T> void RandomArray(T* data, int size);
void PrintTime();

void PrintTime()
{
	int start_s = clock(); //Start the timing

	//This is where a sort would go to be timed
	//Insert sort here
	
	int stop_s = clock(); //Stop the timing once the sort is done
	cout << "\nTime is: ";
    	...
    	//Print the time taken for the sort to run
}
